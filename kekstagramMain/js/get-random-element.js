...
(function() {
	...
	var openUploadForm = function() {
		...
		//console.log(editingForm.querySelector('img'));
		//console.log(imgInput.files[0]);

		//Ждем когда изображение подгрузится и тогда используем его путь
		reader.onload = function(evt) {
			var url = reader.result;
			//console.log(url);
			editingForm.querySelector('img').src = reader.result;
			...
			for (var i = 0; i < effectsItems.length; i++) {
				var url = reader.result;
				//console.log(url);
				effectsItems[i].style.backgroundImage = `url('${reader.result}')`;
				//editingForm.querySelector('img').src = reader.result;
			}

		};
		//в конце обработки превью ставим запуск чтения
